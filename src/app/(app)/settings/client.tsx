...
export default function SettingsPage() {
  ...
  return (
    ...
    >
      ...
      <div className="rounded-[24px] bg-surface/55 backdrop-blur-[16px] border border-line px-4 py-3 flex items-center justify-between shadow-[0_8px_30px_rgba(232,131,127,.14)] select-none">
        <button
          onClick={() => setTab(TABS[(TABS.findIndex((t) => t[0] === tab) + TABS.length - 1) % TABS.length][0])}
          aria-label="Предыдущий раздел"
          className="w-11 h-11 flex items-center justify-center text-ink active:scale-90 transition"
        >
          <motion.span
        ...
        <div className="flex-1 text-center">
          <div className="font-pixel text-[15px] text-ink tracking-wider">
      ...
      <div className={tab === "notify" ? "flex flex-col gap-4" : "hidden"}>
      {/* ===== Раздел: Уведомления ===== */}
      ...
      {me?.role === "OWNER" && (
        <button
          ...
          className="w-full rounded-[24px] bg-surface/55 backdrop-blur-[16px] p-5 shadow-[0_8px_30px_rgba(232,131,127,.14)] flex items-center gap-3 text-left"
        >
      ...
      <div className={tab === "cycle" ? "flex flex-col gap-4" : "hidden"}>
      ...
      <div className={tab === "care" ? "flex flex-col gap-4" : "hidden"}>
